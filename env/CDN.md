---
title: "CDN 配置"
description: "国内与境外智能分流 CDN 加速方案的完整搭建记录，涵盖对象存储服务（七牛云/阿里云 OSS）的存储桶创建与配置、CDN 回源策略与缓存规则设定、域名 DNS 智能解析实现国内外分流、Rclone 命令行工具自动同步本地文件到对象存储的操作流程，适用于个人博客与文档站点的全球访问加速。"
icon: "network-wired"
---
