...
目前使用的 CDN Solution来自：https://www.eallion.com/cdn-cname-cloudflare/

一下is CDN 结构图，目的就is解决全球 CDN 并且成本低
...
不记得is什么时候开始有了a这样的闪念，然后就去search for了一下。
结果发现网上的教程都比较老旧。where讲得比较多的is通过 CloudFlare for SaaS 接入 CNAME，但is都supports普通的域名，并不能接入 R2 或者 Worker。

从功能的优先级上来说，我最需要的is分区解析功能，这就导致不能把域名的 NS 转入 Cloudflare。
Cloudflare 的 DNS 确实非常优秀，但 Cloudflare 不能分区解析，它有 CNAME 拉平功能，不过它会把所有中国大陆地区的 IP 解析到联通。
相反国内的 DNS 服务商的分区解析就做得好，可能也is因为国内的域名更需要这种功能吧。
无法全心全意付出，又想去贴贴 Cloudflare，那就只能搞些奇技淫巧。

2022 年 3 月份，CloudFlare 宣布更改了 CloudFlare for SaaS 的收费Strategy，每个账户可以有 100 个域名free额度，而且超额后每个域名按 0.1 USD/月 收取费用。我们就利用 CloudFlare for SaaS 把域名通过 CNAME 接入 Cloudflare，享受 Cloudflare 强大的边缘Calculation能力。

对于小网站，比如本blogs，以上服务都isfree的，free额度：

- DNSPod：用的专业版，但freeversions也有分区解析
...
关于Tencent云的Configuration略过，这里只讲 Cloudflare 的部分。
前提需要 Cloudflare 账号中已经有a可用的域名。
这个域名用来提供 `回退源` （Fallback Origin），假设这个域名is `example.com` 。

### 创建 R2 并绑定自Definition域名

1. 登录控制面板：https://dash.cloudflare.com/ ，Cloudflare 已supportsChinese locale；
2. 创建 R2 存储桶的Method这里略过，如创建：`r2-blog-test`；
3. in `R2` `设置` `公开访问` `自Definition域` `连接域` 为刚才创建的 R2 添加自Definition域名：
...
2. in该域名的 `SSL/TLS` 中选择 `自Definition主机名`；
...
订阅成功后，先添加 `回退源`：`images.example.com`，这个回源域名is绑定in R2 上的自Definition域名。
...
添加后，需要Validation域名，去自己的域名解析控制台，如 DNSPod ，添加 2 条 TXT 记录。
...
`回退源State` `证书State` 和 `主机名State` 都变成 `有效` 后，就去自己的域名解析控制台添加 CNAME 解析。
...
一般的教程到这里就结束了。
但is这样is访问不了 R2 里面的资源的。
最重要的一步，用 Worker Proxy R2。

### 新建 Worker Proxy R2

官方有Documentation介绍怎么通过 Worker 访问 R2：
...
不用 Wrangle CLI 脚本也可以in后台手动创建 Worker。
...
Deployment成功后返回。
in当前 Worker 的设置中，`Variable` `R2 存储桶绑定` 添加绑定：
...
其实有 R2 就够了，但is可能会因为各种各样的原因需要用到 B2。

其实is差不多的。

Backblaze 官方也有Documentation介绍如何通过 Cloudflare Worker 访问 B2。
...
APP KEY 和 ID 要去 Backblaze 后台generates，`B2_ENDPOINT` 要去自己的 B2 存储桶里查看。
...
- `内容`：填入自己 B2 存储桶分配的 `S3 URL` ，有的教程这里written byis `Friendly URL` ，没必要，还要多一步反代。
...
`Zones` 中的域名为 Backblaze B2 设置的 CNAME nameis什么，那回退源就填什么，如：`b2.example.com`。
参考前文即可。
...
以上算is解决了基本的Issue，但is还有a痛点就is SSL 证书Issue，CF is提供free续签更新永不过期证书的，但is国内 CDN 厂商一般都需要手动上传自己申请的证书。。。这个确实不太方便，所以想着能不能找个Solutions：

::: details 自动更新证书脚本

起因：因为Configuration国内 CDN 基本都is需要手动上传证书，但is我申请的域名证书都is基本三个月保质期的，所以就想着写个自动脚本自动更新证书。
环境：
...
def dogecloud_api(api_path, data={}, json_mode=False):
    ...
    :param api_path:    调用的 API Interface地址，Package含 URL 请求parameters QueryString，for example：/console/vfetch/add.json?url=xxx&a=1&b=2
    :param data:        POST 的数据，dictionary，for example {'a': 1, 'b': 2}，传递此parameters表示不is GET 请求而is POST 请求
    ...
    # 这里替换为你的多吉云永久 AccessKey 和 SecretKey，可in用户中心 - 密钥管理中查看
...
# 下面两个路径就is1panel自动generates的证书路径
...
基本Implementation思路就is先Removed现有证书，然后添加读取的证书，然后上传并激活上传后的证书，这样就Implementation CDN 证书自动Configuration了。可以利用 1panel 自动定时执行脚本功能每隔a月执行一次，以此更新证书。真is又Implementation了a奇奇怪怪的使用小技巧 ✊✊✊
