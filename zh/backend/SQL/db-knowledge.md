---
title: "数据库原理"
description: "数据库原理与应用课程期末考试复习笔记，系统化整理关系代数中选择投影连接等核心运算、SQL 多表连接查询与嵌套子查询语法、从 1NF 到 BCNF 的范式理论与规范化分解方法、事务 ACID 四大特性与并发控制机制、B+ 树索引存储结构以及 ER 图实体关系概念建模设计方法。"
icon: "book-open"
---
...
> #### 已知，R=｛A，B，C｝，函数依赖集为 F = ｛B → C, AC → B｝，判断关系模式是不是 2NF
...
> #### 已知，R=｛A，B，C，D｝，函数依赖集为 F = ｛A → C, AD → B｝，判断关系模式是不是 2NF
...
> #### 已知 R(A, B, C)，其函数依赖集为 F = ｛B → C, AC → B｝；该关系模式是否第 3 范式
...
> #### R(A, B, C, D)，其函数依赖集为 F = ｛AB → C, C → D｝；该关系模式是否第 3 范式
