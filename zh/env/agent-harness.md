...
路由的奖池到底有多大？Fireworks 在 113 个真实编码任务上跑了 18 个模型，再问一个 oracle 路由器能做到什么。最佳单模型：74.1%，每任务 6.52 美元；oracle 路由：97.6%，1.88 美元；只用开源权重模型路由也有 90.3%、1.45 美元——反超全部闭源模型。三个值得记住的发现：

- **几乎没有任务真的需要最贵的模型。** 113 个任务里 94 个的最优选择是单价低于 3 美元的模型；三个 11.5 美元以上的旗舰只在 3 个任务上唯一最优。固定用一个旗舰，是在为多数任务用不到的"通用能力"交保险费。
- **路由池不需要大。** 最佳双模型组合比最佳单模型高 13.1 个点，三模型到 91.2%，扩到 18 个只再涨 6.4 个点。价值来自互补覆盖，不是数量。
